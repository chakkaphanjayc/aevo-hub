...
export const rolePermissionDefaults: Readonly<Record<Role, readonly Permission[]>> = {
  OWNER: permissions,
  ADMIN: permissions.filter((p) => p !== "organization.ownership.transfer" && p !== "organization.delete"),
  ORGANIZATION_MANAGER: [
    "organization.read",
    "organization.update",
    "organization.manage",
  ...
  STAFF: ["store.read", "catalog.read", "order.read", "order.create", "query.read"],
  VIEWER: ["store.read", "order.read", "query.read"]
};

export const platformRoles = [
  "SUPER_ADMIN", "SUPPORT", "OPS", "BILLING_ADMIN", "DEVELOPER", "AUDITOR"
] as const;
...
export interface SetupBookingInput {
  organizationId: string;
  storeId?: string;
  venueName: string;
  businessType: string;
