...
export async function getStoreByCode(
  ...
  const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(storeCode.trim());
  let query = database.client
    .from("stores")
    .select("id,organization_id,code,name,currency,status");

  if (isUuid) {
    query = query.or(`id.eq.${storeCode.trim()},code.eq.${storeCode.trim().toUpperCase()}`);
  } else {
    query = query.eq("code", storeCode.trim().toUpperCase());
  }
