...
export async function listOrganizationDevices(
  ...
  throwDatabaseError(result.error, "organization devices list");
  if (!result.data) return [];
  return (result.data as Row[]).map(mapDevice);
}
...
export async function revokeDevice(database: Database, principal: SessionPrincipal, deviceId: string, storeId?: string): Promise<boolean> {
  let query = database.client
    .from("devices")
