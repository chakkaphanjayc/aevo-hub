...
export function throwDatabaseError(candidate: DatabaseErrorLike, operation: string): never;
export function throwDatabaseError(candidate: DatabaseErrorLike | null | undefined, operation: string): void;
export function throwDatabaseError(candidate: unknown, operation: string): never;
export function throwDatabaseError(candidate: unknown, operation: string): never | void {
  if (!candidate) return;
  const error: DatabaseErrorLike = candidate && typeof candidate === "object" && "message" in candidate
    ? candidate as DatabaseErrorLike
